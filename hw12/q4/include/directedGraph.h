...
std::string toRPN(DirectedGraph* g) {
    ...
    std::function<std::string(int cur)> dfs = [&](int cur) -> std::string {
        ...
        // 先加上左子树的
        if (g->vertices[cur].firstEdge != nullptr) rpn = rpn + dfs(g->vertices[cur].firstEdge->adjVex);
        // 再加上右子树的
        if (g->vertices[cur].firstEdge->next != nullptr) rpn = rpn + dfs(g->vertices[cur].firstEdge->next->adjVex);
        // 最后加上自己的字符
